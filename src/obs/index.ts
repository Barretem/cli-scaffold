 ...
 */
/**
 * 在项目中执行obs build dev/qa命令
 * 读取项目中的.obs.dev/.obs.qa配置的OBS信息（BUILD_OBS_KEY、 BUILD_OBS_SECRET、BUILD_OBS_BUCKET、 BUILD_OBS_ENDPOINT ）
 * 拼接项目中固定的文件目录；
...
export default class Build implements IBuildFunc {
  ...
  constructor(options:IBuildOptions) {
    ...
    this.conf = {
      type,
      configPath: configPath || `./.obs.${type}.env`,
  ...
  async run() {
    ...
    // 将publicPath写入到node环境变量中，并写入到对应的.obs.dev/.obs.qa文件中
  ...
  asyncReadFileAndFormatConfig(): ISPaaSBuildConfig | null {
    ...
    const obsConfigPath = path.resolve(process.cwd(), configPath)
    console.log(`正在读取${obsConfigPath}配置文件`);
    
    const parsed = this.parse(fs.readFileSync(obsConfigPath, { encoding }))
    if(!parsed) {
      console.log(`${chalk.red(`读取${obsConfigPath}配置文件失败，请确认配置是否正确！`)}`);
      return null
    }
    ...
    if(notExitKeys.length) {
      console.log(chalk.red(`配置文件${obsConfigPath}中缺少${notExitKeys.join(',')}的配置`))
      return null
    }
    ...
    console.log(`${chalk.green(obsConfigPath)}配置文件读取完成`);
    return parsed;
  }
   ...
   * @param ossConfig 发布需要用到的OBSConfig文件
   */
  async transportFileToObs(): Promise<boolean> {
    console.log(chalk.green('正在将代码拷贝到OBS服务器，请稍后...'))

    const distDir = path.resolve(process.cwd(), './dist');
    // 读取本地文件列表
    try {
      // object表示上传到OBS的Object名称，localfile表示本地文件或者文件路径
    ...
    this.client = new ObsClient({
      access_key_id: BUILD_OBS_KEY,
      secret_access_key: BUILD_OBS_SECRET,
      server: `https://${BUILD_OBS_ENDPOINT}`
    });
    ...
    return Promise.all(promiseArr).then((res) => {
      ...
      if(status) {
        console.log(chalk.green('项目已经上传到OBS，快点去回归测试一下吧！'));
        console.log(`项目的访问地址为：${chalk.green(this.envConf.BUILD_ACCESS_PATH)}`);
      } else {
        console.log(chalk.red('项目上传到OBS失败，请重试！'));
      }
      return status;
    })
  }
  ...
  uploadFileOneByOne(filePath: string): Promise<boolean> {
    ...
    // 华为云OBS需要将路径前面的/去除
    const obsFileName = fileName.indexOf('/') === 0 ? fileName.slice(1) : fileName
    console.log(`正在将${chalk.green(filePath)}文件上传到${chalk.green(obsFileName)}中...`)
    return this.client.putObject({
      Bucket: BUILD_OBS_BUCKET,
      Key: obsFileName,
      SourceFile: filePath,
  ...
  writePublicPath(): boolean {
    ...
    const fileResultStr = Object.keys(envConf).reduce((str, key) => {
      const shouldNoInEnv = ['BUILD_OBS_KEY', 'BUILD_OBS_SECRET', 'BUILD_OBS_BUCKET', 'BUILD_ACCESS_PATH', 'BUILD_OBS_ENDPOINT'];
  ...
  readEnvFile(): string | null {
    ...
    const obsConfigPath = path.resolve(process.cwd(), '.env')
    console.log('正在读取原来的.env文件...')
    try {
      const str = fs.readFileSync(obsConfigPath, { encoding });
